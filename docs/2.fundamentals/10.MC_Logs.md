...
Where MESSAGE_TYPE can be **INFO, WARN, ERROR or DEBUG**. 
These tags categorize log messages according to their purpose. 

**INFO messages inform the user about Geth's current configuration and status**. 
**WARN messages are for alerting the user to details that affect the way Geth is running**. 
**ERROR messages are for alerting the user to problems**. 
**DEBUG is for messages that are relevant to troubleshooting or for developers working on Geth**.
